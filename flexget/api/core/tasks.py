    ...
    task_execution_input = {
        ...
        'properties': {
            ...
            'second_guess_metadata': {
                'type': 'boolean',
                'default': False,
                'description': "After execution, rerun metadata 'guess' functionality for each entry in 'entry_dump', "
                               "to maximise likelyhood of returning entry metadata",
            },
            'inject': {
                'type': 'array',
                'items': inject_input,
    ...
    if task.stream:
        ...
        if task.stream['args'].get('entry_dump'):
            ...
            task.stream['queue'].put(
                EntryDecoder().encode(
                    {'entry_dump': [e.store for e in entries_to_dump], 'task_id': task.id}
                )
            )
